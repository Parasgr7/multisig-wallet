...
export default function Accounts() {
  ...
  const deposit = async () => {
    ...
    if (selectedToken == "ETH") {
      try {
        await trackPromise(
          state.walletContract.methods
            .deposit(selectedToken, toWei(depositAmount), state.selectedWallet)
            .send({ from: account.data, value: toWei(depositAmount) })
        );
      } catch (e) {
        if (e.code === 4001) {
      ...
      const contract =
        selectedToken == "LINK" ? state.LinkContract : state.DaiContract;
      await trackPromise(
        contract.methods
          .approve(state.walletContract._address, toWei(depositAmount))
      ...
      try {
        ...
        if (e.code === 4001) {
          ...
          toast.error(error_msg.length > 0 ? error_msg : "Error!", {
            hideProgressBar: true,
            theme: "white",
          });
        }
  ...
  const withdraw = async () => {
    ...
    try {
      await trackPromise(
        state.walletContract.methods
          .withdraw(selectedToken, toWei(withdrawAmount), state.selectedWallet)
          .send({ from: account.data })
      );
      const balance = await state.walletContract.methods
        .getBalance(selectedToken, state.selectedWallet)
                  ...
                  return (
                    <>
                      ...
                      >
                        ...
                        <td className="py-4 px-6">{element.sender}</td>
                        <td className="py-4 px-6">{toEther(element.amount)}</td>
                        <td className="py-4 px-6">{element.ticker}</td>
                        <td className="py-4 px-6">{element.action}</td>
