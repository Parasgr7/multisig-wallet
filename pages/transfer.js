...
export default function Transfer() {
  ...
  const { result_length, pending_transactions, cancel_transactions } =
    useTransferRequest();
  ...
  const createTransfer = async () => {
    ...
    !state.web3.utils.isAddress(address)
    ...
    try {
      ...
      setAddress("");
      const balance = await state.walletContract.methods
      ...
      if (e.code === 4001) {
        ...
        var error_msg = JSON.parse(e.message.split("'")[1])["value"]["data"][
          "message"
        ].split("revert")[1];
  ...
  const approveTransRequest = async (transaction_id) => {
    try {
      await trackPromise(
        state.walletContract.methods
          .approveTransferRequest(Number(transaction_id), state.selectedWallet)
          .send({ from: account.data })
      );
  ...
  const cancelTransRequest = async (transaction_id) => {
    try {
      await trackPromise(
        state.walletContract.methods
          .cancelTransferRequest(Number(transaction_id), state.selectedWallet)
          .send({ from: account.data })
      );
        ...
        <div>
          <div className="grid items-center justify-center text-center">
            <h1 className="text-3xl font-light">
              {" "}
              {result_length > 0 ? "Transfer Transactions" : null}
            </h1>
          </div>
              ...
              <ul className="flex flex-wrap -mb-px justify-center items-center">
                <li id="pending" className="w-80">
                  <a
                    className={
                      pendingTab
                        ? "text-blue-600 border-b-2 inline-block p-4 rounded-t-lg border-blue-600 dark:text-blue-500 dark:border-blue-500"
                        : "inline-block p-4 rounded-t-lg border-blue-600 dark:text-blue-500 dark:border-blue-500"
                    }
                    onClick={() => {
                      setPendingTabActive(true);
                      setCancelledTabActive(null);
                    }}
                  >
                ...
                <li id="cancel" className="w-80">
                  <a
                    ...
                    onClick={() => {
                      setCancelledTabActive(true);
                      setPendingTabActive(null);
                    }}
                  >
                    Cancel Transaction
                  </a>
          ...
          <div className="grid items-center justify-center text-center">
              ...
              <table className="text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  {pending_transactions
                    ? pending_transactions.map((element, index) => {
                          ...
                          <>
                            ...
                            >
                              ...
                              <td className="py-4 px-6">
                                {element.sender.slice(0, 7) +
                                  "..." +
                                  element.sender.slice(
                                    element.sender.length - 10
                                  )}
                              </td>
                              <td className="py-4 px-6">
                                {element.receiver.slice(0, 7) +
                                  "..." +
                                  element.receiver.slice(
                                    element.receiver.length - 10
                                  )}
                              </td>
                              <td className="py-4 px-6">
                                {element.ticker == "ETH"
                              ...
                              <td className="py-4 px-6">{element.ticker}</td>
                              <td className="py-4 px-6">
                              ...
                              <td className="py-4 px-6">
                                <button
                                  ...
                                  onClick={() =>
                                    approveTransRequest(element.id)
                                  }
                                >
                                  Approve
                                </button>
            ...
            {cancelledTab && (
              <table className="text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                <tbody>
                  {cancel_transactions
                    ? cancel_transactions.map((element, index) => {
                        var date = new Date(0); // The 0 there is the key, which sets the date to the epoch
                        date.setUTCSeconds(element.timeOfTransaction);
                        return (
                          <>
