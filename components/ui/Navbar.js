        ...
        <div className="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
          <div className="relative flex h-16 items-center justify-between">
                  ...
                  {state.selectedWallet ? (
                    <>
                      ...
                      <label className="dropdown">
                        <div className="dd-button">{selectedToken}</div>
                        <input type="checkbox" className="dd-input" id="test" />

                        <ul className="dd-menu">
                          {state.tokenList
                            ? state.tokenList.map((element, index) => {
                                return (
                                  <li
                                    key={index}
                                    onClick={() => setSelectedToken(element)}
                                  >
                                    {element}
                                  </li>
                                );
            ...
            <div className="absolute inset-y-0 right-0 flex items-center pr-2 sm:static sm:inset-auto sm:ml-6 sm:pr-0">
              <a
                href="#"
                className="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-lg"
              >
                <b>Wallet ID: </b>
                {state.selectedWallet
                  ? state.selectedWallet.slice(0, 5) +
                    "..." +
                    state.selectedWallet.slice(state.selectedWallet.length - 5)
                  : "N/A"}
              </a>
              <a
                href="#"
              ...
              >
                <b>Balance: </b>
                {balance ? balance + " " + selectedToken : "N/A"}
              </a>

              <div className="px-4 py-1 ml-4 text-white border bg-gray-800 border-gray-400 rounded-md">
                {account_address.data
                  ? account_address.data.slice(0, 7) +
                    "..." +
                    account_address.data.slice(account_address.data.length - 7)
                  : "UserID: N/A"}
              </div>
            </div>
          </div>
