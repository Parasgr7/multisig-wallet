...
export const useAccountRequest = () => {
  ...
  const result = account_transactions.data
    ? account_transactions.data.filter(accountTransactions)
    : "";
...
export const useTransferRequest = () => {
  ...
  const pending_transactions = transfer_requests.data
    ? transfer_requests.data.filter(pendingTransactions)
    : "";
  const cancel_transactions = transfer_requests.data
    ? transfer_requests.data.filter(canelTransactions)
    : "";
